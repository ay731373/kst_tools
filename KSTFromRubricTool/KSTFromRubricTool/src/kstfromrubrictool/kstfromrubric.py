...
class GUI(Frame):
    ...
    def get_surmised(self, complexity, item):
        ret_val = []
        for i in complexity:
            if item[1]==i[1]+1:
                ret_val.append((item[0],i[0]))
        return ret_val

    def get_knowledge_space(self, complexity):
        ret_val = []
        for item in complexity:
            ret_val.extend(self.get_surmised(complexity, item))
        return ret_val

    def construct_diff(self):
        complexity = self.load_ks(self.e_ks.get())
        self.txt_diff.delete(1.0,END)
        self.txt_diff.insert(END,""+str(self.get_knowledge_space(complexity)))
